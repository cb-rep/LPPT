...
def train(nb_epoch, batch_size, store_name, resume=False, start_epoch=0, model_path=None):
    ...
    # NOTE 自定义dataset方法
    trainset = CustomImageFolder2(root='./first/a1', transform=transform_train)
    ...
    # )
    net = torch.load('./first_train/model.pth')
    ...
    for epoch in range(start_epoch, nb_epoch):
        ...
        total = 0
        idx = 0
        for batch_idx, (inputs, targets, pos, pos2, is_a) in enumerate(trainloader):
            ...
            for i in range(batch_size):
                if is_a[i] == 'a1' and torch.max(output.data, 1)[1][i].item() != targets[i].item():
                    loss *= 100
                    break
        ...
        # NOTE 记录b的count
        count = test2(netp, batch_size, count, epoch)
        count.to_excel('./' + store_name + '/b数据集每张图片认可数.xlsx')


if __name__ == '__main__':
    train(nb_epoch=100,  # number of epoch
          batch_size=16,  # batch size
          store_name='second_train',  # folder for output
